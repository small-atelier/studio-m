"""
Pyramid edge trim V5 - "V-profile, 3-strand braid" (Blender bpy) - standalone.

Numbered to match trim_v5_braid_3strand.py's motif - same
rail+3-strand-braid border as that file, ported onto this corner-bracket
family's fold/leg/print-orientation scaffolding (see
...
EXPORT_DIR = "/Users/mannil/Desktop/studio-m/TSONS/pyramid/trim/output/edge_v5_braid_3strand"
EXPORT_STL = True
...
# Border: same rail+3-strand-braid motif as trim_v5_braid_3strand.py,
...
def build_zigzag_pieces(length, outer_offset, channel_width, period, n_strands, strand_spacing, strand_width):
    """Builds the braid's zigzag pieces in the leg's own local frame - see
    trim_v5_braid_3strand.py for the full derivation, unchanged
    here."""
...
def build_border_solid(strip_w, strip_l, depth):
    """Border: one straight rail down each long edge, plus a 3-strand
    zigzag braid ribbon that spans the full channel between them -
    unchanged from trim_v5_braid_3strand.py."""
...
def build_and_export_corner():
    ...
    bracket = leg_a
    bracket.name = "trim_edge_braid_3strand"
