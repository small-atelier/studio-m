"""
Pyramid trim strip V4 - "2-strand braid" (Blender bpy) - standalone.

A long straight trim strip, flat-printed: a plain 2mm-thick backing slab
with a raised border rail down each long edge and a zigzag braid ribbon
between them, touching both rails as it alternates - same rail+zigzag-
ribbon language as runes/rune_panel_v5_fishbone_2strand.py (kept visually
consistent with the rest of the board), but as one continuous braid
between two parallel edges instead of a picture-frame around a rectangle.

One of a family of border motifs for the same strip footprint - see
trim_v1_plain.py, _v2_studs.py, _v3_fishbone_spine.py, and
_v5_braid_3strand.py (the same braid, one strand denser) for the others.
...
EXPORT_DIR = "/Users/mannil/Desktop/studio-m/TSONS/pyramid/trim/output/v4_braid_2strand"
EXPORT_STL = True
...
# STRIP_W - 2*RAIL_OUTER_INSET = 14.0mm - much wider than the rune panels'
# 2mm channel, since here the braid IS the field between the borders, not
# a thin ribbon inside a big flat margin.
#
...
def merge_pieces_into_solid(parts, depth, base_name):
    ...
    (only the zigzag's ~20 pieces) rather than folded into one giant union
    with the rails - see rune_panel_v6_fishbone_3strand.py's
...
def build_zigzag_pieces(length, outer_offset, channel_width, period, n_strands, strand_spacing, strand_width):
    ...
    for k in range(n_legs):
        ...
        for j in range(n_strands):
            ...
            # dropped/duplicated geometry elsewhere in this pipeline. Not
            # reachable at ZIGZAG_STRANDS=2 (no centered strand), kept for
            # safety if this gets bumped to an odd count later.
            if offset_index == 0:
                offset_index = 0.15
