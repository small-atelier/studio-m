"""
Pyramid edge trim V3 - "V-profile, fishbone spine" (Blender bpy) - standalone.

Numbered to match trim_v3_fishbone_spine.py's motif - two edge
rails plus a center spine carrying a repeating X-cross of ribs reaching
toward both rails - ported onto this corner-bracket family's fold/leg/
print-orientation scaffolding (see trim_edge_v4_braid_2strand.py for the
full derivation of that scaffolding - unchanged here, only the border
motif differs).
...
EXPORT_DIR = "/Users/mannil/Desktop/studio-m/TSONS/pyramid/trim/output/edge_v3_fishbone_spine"
EXPORT_STL = True
...
# Border: same rail+fishbone-spine motif as
# trim_v3_fishbone_spine.py, unchanged - see that file for the
...
def build_fishbone_ribs(strip_l, rib_length, rib_width, spacing):
    """Builds the spine's rib pairs - unchanged from
    trim_v3_fishbone_spine.py."""
...
def build_border_solid(strip_w, strip_l, depth):
    ...
    X-cross of ribs reaching toward both rails, clipped to the leg's own
    footprint (unchanged from trim_v3_fishbone_spine.py - see
    that file for why the clip is needed)."""
...
def build_and_export_corner():
    ...
    bracket = leg_a
    bracket.name = "trim_edge_fishbone_spine"
