"""
Rune panel batch V6 - "fishbone, 3 strands" (Blender bpy) - standalone.

Same as rune_panel_v5_fishbone_2strand.py (mask -> smoothed grid -> extrude
glyph, flat direct-to-plate orientation, 18x27mm panel, 10x15mm glyph
footprint, rail + zigzag ribbon border), except each zigzag leg is drawn as
3 parallel strands instead of 2 (ZIGZAG_STRANDS), with a tighter strand
spacing so the ribbon's overall width comes out about the same as the
2-strand version - see the border config comment below.
...
EXPORT_DIR = "/Users/mannil/Desktop/studio-m/TSONS/runes/output/v6_fishbone_3strand"
EXPORT_STL = True
...
# ------------------------------------------------------------
# Ornamental border: "rail + zigzag ribbon" - two straight rule lines
...
# ZIGZAG_STRANDS parallel strands (a ribbon, not a single line). This is
# the 3-strand variant of rune_panel_v5_fishbone_2strand.py - ZIGZAG_STRANDS=3,
# with ZIGZAG_STRAND_SPACING widened a touch (0.6 -> 0.55) and
# ZIGZAG_STRAND_WIDTH narrowed (0.4 -> 0.35) so the 3 strands read as
# distinct parallel lines instead of merging into a solid filled wedge
# (a straight port of the 2-strand spacing/width overlapped at 3-up).
#
...
# by period/2) but it's not visually distinguishable. At 3 strands/leg this
# is ~146 border pieces total - see build_border_solid for how those get
# assembled without that piece count wrecking reliability.
# ------------------------------------------------------------
...
def merge_pieces_into_solid(parts, depth, base_name):
    ...
    one watertight solid via real booleans - see build_border_solid for why
    keeping each such chain short (rather than one chain for everything)
...
def build_zigzag_edge_canonical(length, outer_offset, channel_width, period, n_strands, strand_spacing, strand_width):
    ...
    for k in range(n_legs):
        ...
        for j in range(n_strands):
            ...
            # For odd n_strands, the centered strand (offset_index == 0)
            # would have its leg endpoints land exactly on the outer/inner
            # rail centerlines - an exact-coincidence degenerate boolean
            # input (same class as the flush-boundary issue EMBED_DEPTH
            # avoids above) that made build_border_solid's ~146 sequential
            # unions intermittently drop chunks of geometry (verified via
            # STL volume: some panels came out at ~200 instead of ~1100).
            # Nudging it off exact zero breaks the coincidence; imperceptible
            # at this scale.
            if offset_index == 0:
                offset_index = 0.15
...
def build_border_solid(panel_w, panel_h, depth):
    """Border: two straight rails (outer at RAIL_OUTER_INSET, inner at
    ...
    At 3 strands/leg this pipeline's original construction - every piece
    (rails + all 4 edges' zigzags, ~146 of them at ZIGZAG_PERIOD=4.0) fed
    into one long sequential-union chain, same as v2-v5 - turned out to be
    past Blender's EXACT solver's reliability limit: verified via STL
    volume across the batch, most panels were silently missing or
    duplicating chunks of border geometry, not just the one glyph that was
    visibly broken. Splitting into short independent chains (rails, and
    each of the 4 edges' zigzag separately - at most ~42 pieces each, since
    each edge doesn't overlap the others except at the corners) merged at
    the very end fixed it, without needing to reduce the pattern's density.
    See build_glyph_stamp_mesh's docstring for the general class of issue -
    booleans against a pile of separately-overlapping shells as a single
    cutter is what fragmented an earlier version of this pipeline; this is
    the same problem one level up, from chaining too many real booleans in
    a row rather than from an unmerged pile."""
    ...
    for inset in (RAIL_OUTER_INSET, inner_inset):
        ...
        half_t = RAIL_WIDTH / 2
        rail_parts.append((0.0, rail_h / 2 - half_t, rail_w, RAIL_WIDTH, 0.0))
        rail_parts.append((0.0, -(rail_h / 2 - half_t), rail_w, RAIL_WIDTH, 0.0))
        rail_parts.append((rail_w / 2 - half_t, 0.0, RAIL_WIDTH, rail_h, 0.0))
        rail_parts.append((-(rail_w / 2 - half_t), 0.0, RAIL_WIDTH, rail_h, 0.0))
    ...
    group_solids = [
        merge_pieces_into_solid(rail_parts, depth, "rails"),
        merge_pieces_into_solid(top, depth, "top_zigzag"),
        merge_pieces_into_solid(bottom, depth, "bottom_zigzag"),
        merge_pieces_into_solid(right, depth, "right_zigzag"),
        merge_pieces_into_solid(left, depth, "left_zigzag"),
    ]
    solid = group_solids[0]
    for other in group_solids[1:]:
        apply_boolean(solid, other, 'UNION')
