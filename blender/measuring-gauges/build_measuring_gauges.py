...
def shift_to_bed_z(objs):
    """Translate every object in objs by the SAME +Z offset (-BASE_Z), then clamp any
    still-negative vertex up to exactly 0. The triangular pieces are already correctly
    ORIENTED as built (flat face down, apex up), but their base face sits at BASE_Z, a
    negative Z value - left as-is, a slicer taking a 3MF's raw coordinates at face
    value (no auto-drop-to-bed the way loose STLs sometimes get) finds nothing at
    layer zero, reported as "one object has an empty first layer."

    -BASE_Z alone isn't quite enough, though: it's the BASE shell's own true minimum,
    but not the INLAY's. Rotating a recessed marking's small depth offset by 120°/240°
    (to place it on face 1/2) mixes that offset with the marking's own width position,
    and for markings near the shared edge between two faces this can land the marking
    a hair below BASE_Z itself (~0.5mm, confirmed by checking the exported STL
    directly). Shifting by that ACTUAL combined minimum instead would fix the inlay
    but leave the base - the part that actually needs to grip the bed - sitting
    0.5mm proud of Z=0 instead, which is worse, not better. So: shift by the base's
    own intended reference (-BASE_Z), then clamp only the stray negative leftovers
    (a tiny sliver of one rotated marking, not the base) up to 0 rather than letting
    that edge case redefine where the whole part sits."""
    for obj in objs:
        bm = bmesh.new()
        bm.from_mesh(obj.data)
        for v in bm.verts:
...
def export_tri_segment(name, base, inlay):
    ...
    render_face entirely; render_segments/render_charge_stick_tri render separately,
    after both are built. shift_to_bed_z runs here (export-time only) rather than in
    the build functions, so the internal geometry (BASE_Z, TRI_BADGE_X's own render
    camera targets, etc.) keeps using its original, already-verified coordinates."""
    shift_to_bed_z([base, inlay])
    # The clamp-to-0 step in shift_to_bed_z moves vertices directly, bypassing the
    # boolean pipeline entirely - re-check manifold status here rather than trusting
    # it stayed clean, since flattening a vertex can in principle collapse a face.
    for obj, label in ((base, f"{name}_base (post-shift)"), (inlay, f"{name}_inlay (post-shift)")):
        nm = nonmanifold_fraction(obj)
        print(f"{label}: non-manifold {nm:.5f}")
        assert nm == 0.0, f"{label}: shift_to_bed_z's clamp introduced non-manifold geometry"
    apply_color(base, "base_black", BASE_COLOR)
    apply_color(inlay, "inlay_white", INLAY_COLOR)
    export_stl(base, f"{name}_base.stl")
    export_stl(inlay, f"{name}_inlay.stl")
...
def main():
    ...
    # Render BEFORE export_tri_segment - it shifts geometry to sit on Z=0 for
    # print/slicing, but the render cameras target the pre-shift coordinates
    # (BASE_Z, SEGMENT_LEN, etc.), same "render before reorienting" order the flat
    # family's own export_piece already uses.
